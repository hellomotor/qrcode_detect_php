...
Ref<LuminanceSource> ImageReaderSource::create(const unsigned char *ptr, size_t size, int imageType) {
	...
	if (imageType == 0) {
		...
		{ unsigned w, h;
			unsigned error = lodepng::decode(out, w, h, ptr, size);
			if (error)  {
				ostringstream msg;
				msg << "Error while lodepng::decode '" << lodepng_error_text(error) << "'";
		...
		char *buffer = reinterpret_cast<char*>(jpgd::decompress_jpeg_image_from_memory(
					ptr, size, &width, &height, &comps, 4));
		image = zxing::ArrayRef<char>(buffer, 4 * width * height);
	}
	if (!image) {
		ostringstream msg;
		msg << "Loading from memory failed.";
		throw zxing::IllegalArgumentException(msg.str().c_str());
	}
