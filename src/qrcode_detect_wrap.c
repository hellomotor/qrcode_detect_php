...
/* header section */
ZEND_BEGIN_MODULE_GLOBALS(qrcode_detect)
const char *error_msg;
int error_code;
ZEND_END_MODULE_GLOBALS(qrcode_detect)
ZEND_DECLARE_MODULE_GLOBALS(qrcode_detect)
#ifdef ZTS
#define SWIG_ErrorMsg() TSRMG(qrcode_detect_globals_id, zend_qrcode_detect_globals *, error_msg )
#define SWIG_ErrorCode() TSRMG(qrcode_detect_globals_id, zend_qrcode_detect_globals *, error_code )
#else
#define SWIG_ErrorMsg() (qrcode_detect_globals.error_msg)
#define SWIG_ErrorCode() (qrcode_detect_globals.error_code)
#endif

static void qrcode_detect_init_globals(zend_qrcode_detect_globals *globals ) {
  globals->error_msg = default_error_msg;
  globals->error_code = default_error_code;
}
static void qrcode_detect_destroy_globals(zend_qrcode_detect_globals * globals) { (void)globals; }
...
ZEND_NAMED_FUNCTION(_wrap_swig_qrcode_detect_alter_newobject) {
  zval **args[2];
  swig_object_wrapper *value;
  int type;
  int thisown;
...
ZEND_NAMED_FUNCTION(_wrap_swig_qrcode_detect_get_newobject) {
  zval **args[1];
  swig_object_wrapper *value;
  int type;
...
#define SWIG_name  "qrcode_detect"
#ifdef __cplusplus
extern "C" {
#endif
#include "php.h"
...
#endif
zend_module_entry qrcode_detect_module_entry = {
#if ZEND_MODULE_API_NO > 20010900
    STANDARD_MODULE_HEADER,
#endif
    (char*)"qrcode_detect",
    qrcode_detect_functions,
    PHP_MINIT(qrcode_detect),
    PHP_MSHUTDOWN(qrcode_detect),
    PHP_RINIT(qrcode_detect),
    PHP_RSHUTDOWN(qrcode_detect),
    PHP_MINFO(qrcode_detect),
#if ZEND_MODULE_API_NO > 20010900
    NO_VERSION_YET,
#endif
    STANDARD_MODULE_PROPERTIES
};
zend_module_entry* SWIG_module_entry = &qrcode_detect_module_entry;

#ifdef __cplusplus
extern "C" {
#endif
SWIGEXPORT zend_module_entry *get_module(void) { return &qrcode_detect_module_entry; }
#ifdef __cplusplus
}
#endif

#define SWIG_php_minit PHP_MINIT_FUNCTION(qrcode_detect)
/* -----------------------------------------------------------------------------
 * Type initialization:
...
/* oinit subsection */
ZEND_INIT_MODULE_GLOBALS(qrcode_detect, qrcode_detect_init_globals, qrcode_detect_destroy_globals);
...
/* shutdown section */
#ifdef ZTS
    ts_free_id(qrcode_detect_globals_id);
#endif
    return SUCCESS;
}
